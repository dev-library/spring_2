	...
	<table class="table table-hover">
		...
		<tbody>
			<c:forEach var="board" items="${boardList }">
				<tr>
					<td>${board.bno }</td>
					<td><a href="http://localhost:8181/board/detail?bno=${board.bno }&keyword=${pageMaker.cri.keyword}&searchType=${pageMaker.cri.searchType}&page=${pageMaker.cri.page}">${board.title }</a></td>
					<td>${board.writer }</td>
					<td>${board.regDate }</td>
